...
std::string vulkan_api_version_to_string(const std::uint32_t version)
{
...
std::string vulkan_device_type_to_string(const VkPhysicalDeviceType type)
{
    switch (type) {
...
void throw_if_vk_failed(const VkResult result, const std::string_view operation)
{
    ...
    throw std::runtime_error(
        std::string(operation) + " failed: " + vk_result_to_string(result)
    );
}
